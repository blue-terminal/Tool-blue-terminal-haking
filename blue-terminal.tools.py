...
from docx import Document 
import colorama
tabella=Table(title="strumenti haking di blue_terminal",style="yellow",title_justify="center")
...
if utente==1:
    ...
    tabella.add_column(f"{1}server")
    tabella.add_column(f"{2}numero clienti")
    if utente==1:
        subprocess.run(["ping","-n","10","192.168.1.1"])
        response=subprocess.run(["ping","-n","10","192.168.1.1"],stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if response.returncode==0:
            print("Server attivo")
            for line in response.stdout.decode().split("\n"):
                if "TTL" in line:
                    numero_clienti=line.split()[3]
                    print(f"Numero clienti: {numero_clienti}")
                    tabella.add_row([f"Server 1", numero_clienti])
        else:
            print("Server non attivo")
            tabella.add_row([f"Server 1", "0"])
    elif utente==2: 
        subprocess.run(["ping","-n","10","192.168.1.2"])
        response=subprocess.run(["ping","-n","10","192.168.1.2"],stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if response.returncode==0:
            print("Server attivo")
            for line in response.stdout.decode().split("\n"):
                if "TTL" in line:
                    numero_clienti=line.split()[3]
                    print(f"Numero clienti: {numero_clienti}")
                    tabella.add_row([f"Server 2", numero_clienti])
        else:
            print("Server non attivo")
            tabella.add_row([f"Server 2", "0"])
